...
ASSETS_DIR="$ROOT_DIR/assets/tiff"
WIDTH="${1:-120}"
HEIGHT="${2:-60}"
...
if [[ ! -d "$ASSETS_DIR" ]]; then
  echo "assets/tiff directory not found at $ASSETS_DIR" >&2
  exit 1
fi

shopt -s nullglob
files=("$ASSETS_DIR"/*.tif)
if (( ${#files[@]} == 0 )); then
  echo "No .tif files found in $ASSETS_DIR" >&2
  exit 1
fi
...
for file in "${files[@]}"; do
  ...
  if [[ "$size" -le 8 ]]; then
    echo "=== $file ==="
    echo "SKIPPED: file too small to be a valid TIFF"
    continue
  fi
  echo
  echo "=== $file ==="
