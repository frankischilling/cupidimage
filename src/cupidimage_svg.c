...
static int svg_strcasestr_simple(const char *haystack, const char *needle) {
...
static char *svg_strdup(const char *s) {
    if (!s) {
        return NULL;
    }
    size_t len = strlen(s);
...
static char *svg_strndup(const char *s, size_t len) {
    if (!s) {
        return NULL;
    }
    char *out = (char *)malloc(len + 1);
...
static void svg_matrix_identity(svg_matrix *m) {
    m->a = 1.0f;
    m->b = 0.0f;
    m->c = 0.0f;
    m->d = 1.0f;
...
static void svg_matrix_multiply(const svg_matrix *a, const svg_matrix *b, svg_matrix *out) {
    svg_matrix r;
    r.a = a->a * b->a + a->c * b->b;
    r.b = a->b * b->a + a->d * b->b;
    r.c = a->a * b->c + a->c * b->d;
...
static void svg_matrix_translate(svg_matrix *m, float tx, float ty) {
    svg_matrix t;
...
static void svg_matrix_scale(svg_matrix *m, float sx, float sy) {
    svg_matrix s;
...
static void svg_matrix_rotate(svg_matrix *m, float angle_deg) {
...
static void svg_matrix_skewx(svg_matrix *m, float angle_deg) {
    float r = angle_deg * (float)M_PI / 180.0f;
    svg_matrix t;
    svg_matrix_identity(&t);
    t.c = tanf(r);
...
static void svg_matrix_skewy(svg_matrix *m, float angle_deg) {
    float r = angle_deg * (float)M_PI / 180.0f;
    svg_matrix t;
    svg_matrix_identity(&t);
    t.b = tanf(r);
...
static int svg_matrix_invert(const svg_matrix *m, svg_matrix *out) {
...
static void svg_matrix_transform_point(const svg_matrix *m, float *x, float *y) {
...
static int svg_is_name_char(int c) {
    return isalnum(c) || c == ':' || c == '-' || c == '_';
}

static void svg_lowercase(char *s) {
    if (!s) {
        return;
    }
    for (; *s; s++) {
...
static const char *svg_local_name(const char *name) {
...
static int svg_parse_transform(const char *value, svg_matrix *out) {
    ...
    while (*p) {
        ...
        if (strncmp(p, "matrix", 6) == 0) {
            ...
            svg_parse_number(&p, &tx);
            if (!svg_parse_number(&p, &ty)) {
                ty = 0.0f;
            }
            svg_matrix t;
            svg_matrix_identity(&t);
            ...
            svg_parse_number(&p, &sx);
            if (!svg_parse_number(&p, &sy)) {
                sy = sx;
            }
            svg_matrix t;
            svg_matrix_identity(&t);
            ...
            p++;
            float angle = 0.0f;
            if (!svg_parse_number(&p, &angle)) {
                return 0;
            }
            float cx = 0.0f, cy = 0.0f;
            int has_center = svg_parse_number(&p, &cx);
            if (has_center) {
                if (!svg_parse_number(&p, &cy)) {
                    cy = 0.0f;
                }
            }
            svg_matrix t;
...
static int svg_css_parse(svg_css *css, const char *text) {
    ...
    while (*p) {
        ...
        char *selectors_str = p;
        *brace = '\0';
        char *end = strchr(brace + 1, '}');
        if (!end) {
            break;
        }
        *end = '\0';
        ...
        if (rule.selector_count > 0 && svg_css_parse_decls(&rule, brace + 1)) {
...
static int svg_parse_preamble(const unsigned char *data, size_t size, svg_preamble *pre, svg_defs *defs,
                              svg_css *css, float dpi, char *err, size_t errcap) {
    if (!data || !pre) {
    ...
    while (svg_next_tag(&p, &tag)) {
        ...
        if (current_grad && !tag.is_end && strcmp(local, "stop") == 0) {
            float off = 0.0f;
            uint32_t color = 0;
            if (svg_stop_parse(&tag, &off, &color)) {
...
static uint32_t svg_gradient_stop_color(const svg_gradient *g, float t) {
    ...
    for (int i = 0; i < g->stop_count - 1; i++) {
        ...
        if (t >= o0 && t <= o1) {
            ...
            return (uint32_t)((r << 24) | (gg << 16) | (b << 8) | a);
        }
    }
    return g->stops[g->stop_count - 1].color;
}
...
static void svg_apply_filter_ops(const svg_filter *filter, svg_render_ctx *ctx, uint8_t *buf, float base_len,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (op->type == SVG_FILTER_OP_BLUR) {
            ...
            memset(buf, 0, size);
            if (op->image_href && *op->image_href && op->image_href[0] != '#') {
                if (svg_strncasecmp(op->image_href, "http", 4) != 0 &&
                    svg_strncasecmp(op->image_href, "data:", 5) != 0) {
                    cupidimage_image img;
                    char errbuf[128];
                    if (cupidimage_load_image_file(op->image_href, &img, errbuf, sizeof(errbuf))) {
...
static int svg_render(svg_render_ctx *ctx, const unsigned char *data, size_t size, char *err, size_t errcap,
    ...
    svg_css css;
    svg_preamble pre;
    if (!svg_parse_preamble(data, size, &pre, &defs, &css, dpi, err, errcap)) {
        svg_defs_free(&defs);
        svg_css_free(&css);
        return 0;
    }
    ...
    while (svg_next_tag(&p, &tag)) {
        ...
        if (!root_pushed) {
            ...
            const char *root_transform_attr = svg_get_attr(&tag, "transform");
                ...
                if (!tag.is_self_closing) {
                    if (need_fill) {
                        ...
                        if (fill_ctx.hi_rgba) {
                            ...
                            char *tp = p;
                            svg_render_text_element(&fill_ctx, &fill_style, &m, &tag, &tp, &defs, &css,
                    ...
                    if (need_stroke) {
                        ...
                        if (stroke_ctx.hi_rgba) {
                            ...
                            stroke_style.fill_pattern = NULL;
                            char *tp = p;
                            svg_render_text_element(&stroke_ctx, &stroke_style, &m, &tag, &tp, &defs, &css,
                                                    text_stack, text_depth, base_len, dpi, NULL);
                            stroke_buf = stroke_ctx.hi_rgba;
                        }
                    }
                    char *tp = p;
                    svg_render_text_element(&tmp_ctx, &style, &m, &tag, &tp, &defs, &css,
...
int cupidimage_load_svg_file(const char *path, cupidimage_image *out,
    ...
    int ok = cupidimage_load_svg(buf, (size_t)fsize, out, err, errcap);
    free(buf);
    return ok;
}
